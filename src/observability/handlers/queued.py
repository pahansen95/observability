  ...
  def stop(self) -> None:
    ...
    if self._worker:
      ...
      if self._worker.is_alive() and __debug__:
        print(f"QueuedHandler worker did not stop within {self.timeout}s", file=sys.stderr)
  ...
  def __call__(self, event: EventDict) -> None:
    ...
    try:
      ...
      if __debug__ and self._dropped_count == 1:
        print("QueuedHandler: Queue full, dropping events", file=sys.stderr)
    ...
    while not self._stop_event.is_set():
      try:
        ...
        try:
          ...
          safe_handler_call(f"QueuedHandler.{self.wrapped.__class__.__name__}", "processing event", e)
  ...
  def _drain_queue(self) -> None:
    ...
    while not self._queue.empty() and (time.time() - start_time) < self.timeout:
      try:
        ...
        safe_handler_call("QueuedHandler.drain", "processing remaining event", e)

    if __debug__ and drained > 0:
      print(f"QueuedHandler: Drained {drained} events on shutdown", file=sys.stderr)
